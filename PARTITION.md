...
> **方案**：Bootloader + **APP_A（唯一运行槽）** + **APP_B（厂测存储）** + NVS（**纯片内**）  
...
地址          大小      分区          说明
───────────  ────────  ────────────  ──────────────────────────
0x00000000   16 KB     bootloader    复位入口；校验 APP_A
0x00004000  116 KB     app_a         唯一应用运行槽（主业务固件）
0x00021000  116 KB     app_b         厂测镜像存储（非运行区）
0x0003E000    8 KB     nvs           参数存储（不参与链接）
0x00040000   (末尾)
```

**与旧「片上 A/B 乒乓」差异**：Boot **始终跳转 APP_A**；APP_B **永不作为运行槽**，仅存放厂测镜像。

---

## APP_B 逻辑（116 KB @ `0x00021000`）

| 用途 | 说明 |
|------|------|
| `factory` | 厂测固件预烧存储；经 `ftmenter` 由 Boot 搬运至 APP_A 后运行 |
...
#define FLASH_APP_B_SIZE        0x0001D000U   /* 116 KB — 厂测存储 */
...
| 厂测切换 | `ftmenter`：校验 APP_B 厂测镜像 → Boot 搬运 B→A |
...
| 2026-06-12 | **改为 APP_A 单槽运行 + APP_B 厂测存储** |
| 2026-06-12 | 移除外扩 Flash 方案，**纯片内**四段布局 |
| 2026-07-08 | 移除 OTA 相关描述 |
