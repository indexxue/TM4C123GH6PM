...
function Build-FirmwareTarget {
    ...
    if ($CheckImageSize) {
        & python (Join-Path $ProjectRoot "scripts\check-image-size.py") $Bin --max $MaxImageSize
...
switch ($Target) {
    ...
    "factory" {
        Invoke-AppCodegen
        Build-FirmwareTarget -Name "factory" -LdScript (Join-Path $LdDir "factory.ld") -Sources (Get-AppSources) -ExtraDefines @("-DFLASH_FACTORY_SLOT") -CheckImageSize
    }
    "all" {
        Invoke-AppCodegen
        Build-FirmwareTarget -Name "bootloader" -LdScript (Join-Path $LdDir "bootloader.ld") -Sources (Get-BootloaderSources) -CheckImageSize -MaxImageSize (16 * 1024)
