...
DEFAULT_CFG = ROOT / ".syscfg" / "project.json"
...
def main() -> None:
    ...
    inc_dir = pathlib.Path(args.inc)
    if not cfg_path.exists():
        print("[SKIP] no project.json")
        return
    ...
    L = []
    L.append("/* Auto-generated peripheral init by gen-car-config.py */")
    ...
    for u in cfg.get("uart", []):
        ...
        L.append("}")
        L.append("")
        L.append("void UART_Putc(char c) {")
        L.append("    UARTCharPut(%s_BASE, c);" % u["module"])
        L.append("}")
        L.append("void UART_Puts(const char* s) {")
        L.append("    while (*s) { UARTCharPut(%s_BASE, *s++); }" % u["module"])
        L.append("}")
        L.append("int UART_Getc(char *c) {")
        L.append("    if (UARTCharsAvail(%s_BASE)) {" % u["module"])
        L.append("        *c = (char)UARTCharGetNonBlocking(%s_BASE);" % u["module"])
        L.append("        return 1;")
        L.append("    }")
        L.append("    return 0;")
        L.append("}")
    ...
    for d in cfg.get("uart_debug", []):
        ...
        L.append("}")
        L.append("int UART_Debug_Getc(char *c) {")
        L.append("    if (UARTCharsAvail(%s_BASE)) {" % d["module"])
        L.append("        *c = (char)UARTCharGetNonBlocking(%s_BASE);" % d["module"])
        L.append("        return 1;")
        L.append("    }")
        L.append("    return 0;")
        L.append("}")
    ...
    for a in cfg.get("adc", []):
        ...
        L.append("    ADCProcessorTrigger(%s_BASE, 3);" % a["module"])
        L.append("}")
        L.append("")
    ...
    (src_dir / "peripheral.c").write_text("\n".join(L), encoding="utf-8")

    H = ['#ifndef PERIPHERAL_H', '#define PERIPHERAL_H', '#include <stdint.h>']
    ...
    H.append('')
    H.append('void Motor_Init(void);')
    H.append('void Encoder_Init(void);')
    H.append('void UART_Init(void);')
    H.append('void UART_Putc(char c);')
    H.append('void UART_Puts(const char* s);')
    H.append('int UART_Getc(char *c);')
    H.append('void I2C_Init(void);')
    H.append('void ADC_Init(void);')
    H.append('void SSI_Init(void);')
    ...
    H.append('int UART_Debug_Getc(char *c);')
    H.append('#endif')

    (inc_dir / "peripheral.h").write_text("\n".join(H), encoding="utf-8")
    print("  -> Common/src/peripheral.c")
    print("  -> Common/inc/peripheral.h")
