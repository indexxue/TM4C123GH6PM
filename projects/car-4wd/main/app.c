...
#define APP_EVT_STACK_WORDS         (1024U)
...
static bool_t s_camera_spi_inited;
...
static void app_user_init(void)
{
    ...
    /* 循迹 ADC 已在 Start_Init() → Line_Init() 完成，勿重复 init */
    ...
    /* Camera SPI 延后到开机动画结束后再启，避免 SSI ISR 打断 WS2812 位时序 */
...
static void app_on_timer(void)
{
    ...
    /*
     * Camera SSI 从机：开机动画结束后再启；遥控会话中延后，
     * 避免与流式 DRIVE 抢 CPU（曾在 scene done 后立刻失控）。
     */
    if (!s_camera_spi_inited && !led_scene_is_active() &&
        (proto_host_is_linked() == FALSE)) {
        if (device_profile_board_wants(DEVICE_BOARD_MASK_PERIPH)) {
            st = camera_spi_init();
            if (st != STATUS_OK) {
        ...
        s_camera_spi_inited = TRUE;
    }

    proto_telemetry_tick(APP_CTRL_PERIOD_MS);

    if (s_camera_spi_inited) {
        camera_spi_poll();
    }

    proto_link_tick(APP_CTRL_PERIOD_MS);
...
static void app_button_notify(btn_id_e id, const char *name, btn_permission_e permission, btn_event_e event)
{
    ...
    if ((event == BTN_EVENT_SINGLE_CLICK) || (event == BTN_EVENT_LONG_PRESS)) {
...
static void app_tmr_tick_task(void *arg)
{
    ...
    static uint32_t s_alive_log_ms;
    ...
    for (;;) {
        for (i = 0U; i < APP_CTRL_PERIOD_MS; i++) {
            ...
            s_alive_log_ms++;
            if (s_alive_log_ms >= 5000U) {
                s_alive_log_ms = 0U;
                if (device_profile_platform_wants(DEVICE_PLATFORM_MASK_LOG)) {
                    LOG_INFO("app:alive tmr heap=%u",
                             (unsigned)xPortGetFreeHeapSize());
                }
            }
        }
